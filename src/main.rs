...
fn maaaain() -> Result<(), Box<dyn Error>> {
    ...
    while !window.window_handle().should_close() {
        ...
        //     //unsafe { gl::DrawArrays(gl::TRIANGLES, 0, 32) }
        //     //texture_gato.unbind();
...
fn main() {
    let mut w = Window::new(500, 300, "Hola");
    unsafe {
        gl::ClearColor(154. / 258., 127. / 258., 174. / 258., 1.0);
    }
    w.glfw_handle_mut().set_time(0.);
    while !w.window_handle().should_close() {
